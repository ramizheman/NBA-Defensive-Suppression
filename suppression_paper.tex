...
\textbf{Replication.} The primary DiD specification requires only a flat table with game, team, play type, half, and points columns and does not require graph traversal. Frozen half-level summaries, holdout trigger tables, analysis scripts, and result files for the matched-pair specification are in the repository above. Rebuilding the possession-level dataset from the public API proceeds in three stages: (1)~prefetch and cache raw API responses; (2)~classify possessions; (3)~aggregate to the half-level tables already posted.
